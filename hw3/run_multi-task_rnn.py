...
tf.app.flags.DEFINE_string("predict", False, "To predict testing file only")
tf.app.flags.DEFINE_string("answer_file", "answer.txt", "answer file for prediction")
...
def dump_intent(intent, filename):
...
def train():
  ...
  current_intent_test_out_file = './intent.txt'
  ...
  with tf.Session() as sess:
    ...
    if FLAGS.predict == False:
      while model.global_step.eval() < FLAGS.max_training_steps:
        ...
        if current_step % FLAGS.steps_per_checkpoint == 0:
          ...
          # Run evals on development/test set and print the accuracy.
              ...
              if task['intent'] == 1:
                ...
                if mode == 'Test':
                  ...
                  dump_intent(hyp_label_list, intent_out_file)
              if task['tagging'] == 1:
                if mode == 'Eval':
                    taging_out_file = current_taging_valid_out_file
                elif mode == 'Test':
                    taging_out_file = current_taging_test_out_file
                tagging_eval_result = conlleval(hyp_tag_list, ref_tag_list, word_list, taging_out_file, mode)
          ...
          if task['tagging'] == 1 and valid_tagging_result['f1'] > best_valid_score:
            ...
            subprocess.call(['mv', './tagging.txt', './tagging' + '.best_f1_valid_%.2f.txt' % best_valid_score])

          if task['intent'] == 1 and valid_accuracy > best_valid_acc:
            best_valid_acc = valid_accuracy
            subprocess.call(['mv', './intent.txt', './intent' + '.best_f1_valid_%.2f.txt' % valid_accuracy])
      ...
      if task['tagging'] == 1:
        ...
        # save the best output file
        subprocess.call(['mv', current_taging_valid_out_file, current_taging_valid_out_file + '.best_f1_%.2f' % best_valid_score])
        subprocess.call(['mv', './tagging.txt', FLAGS.answer_file])

      if task['intent'] == 1:
        best_valid_acc = valid_accuracy
        subprocess.call(['mv', './intent.txt', FLAGS.answer_file])
