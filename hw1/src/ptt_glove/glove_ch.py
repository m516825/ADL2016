...
class Data(object):
	...
	def cal_weight(self):
		...
		weight = self.train[:, 2][None,:].T
		max_index = np.where(weight >= 100.)[0]
		weight = weight/100.
		weight = np.power(weight, 3./4.)
		weight[max_index] = 1.
	...
	def next_batch(self, size):
		...
		if self.current + size < self.length:
			...
			weight = self.weight[self.current : self.current + size]
			...
			weight = self.weight[self.current:]
...
def arg_parse():
	...
	parser.add_argument('--train', default='./cooccur_ch.npz.npy', type=str)
	parser.add_argument('--vocab', default='./vocab_ch.out', type=str)
	parser.add_argument('--vector', default='./g_ch_vector', type=str)
...
def main():
	...
	wi, wj = glove_model(args=args ,dat=dat, iteration=20, batch_size=100, learning_rate=0.09, alpha=0.75, x_max=100, vector_size=128, vocab_size=len(v2i))
