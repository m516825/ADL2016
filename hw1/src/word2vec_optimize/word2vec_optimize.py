...
class Word2Vec(object):
  ...
  def dumpVector(self):
    ...
    nemb = (tf.nn.l2_normalize(self._w_in, 1)).eval()
    opts = self._options
    with open(os.path.join(opts.save_path, "vector.txt"), "w") as f:
      for i in xrange(opts.vocab_size):
        vocab_word = tf.compat.as_text(opts.vocab_words[i]).encode("utf-8")
        vec = ' '.join([str(val) for val in nemb[i]])
        f.write(vocab_word+' '+vec+'\n')
